<nav x-data="{ open: false }" class="bg-ivory/85 backdrop-blur border-b-2 border-ink sticky top-0 z-50">
        ...
        <div class="flex justify-between h-16 items-center">
            <div class="flex items-center gap-8">
                ...
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-lg bg-lime border-2 border-ink flex items-center justify-center font-display font-bold text-ink">S</div>
                    <span class="font-display text-lg font-bold tracking-tight hidden sm:block">Sales<em class="not-italic text-salmon">Forge</em></span>
                </a>
            ...
            <div class="hidden sm:flex items-center gap-3">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="flex items-center gap-2 pl-1.5 pr-3 py-1.5 rounded-full text-sm font-semibold border-2 border-ink hover:bg-lime/40 transition-all duration-150">
                            <div class="w-7 h-7 bg-ink rounded-full flex items-center justify-center text-lime text-xs font-bold">
                                {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                            </div>
                            <span class="text-ink">{{ Str::limit(Auth::user()->name, 14) }}</span>
                            <svg class="w-3.5 h-3.5 text-ink/60" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                    </x-slot>
            ...
            <div class="flex items-center sm:hidden">
                <button @click="open = ! open" class="p-2 rounded-full border-2 border-ink text-ink hover:bg-lime/40 transition-all duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden border-t-2 border-ink bg-ivory">
        ...
        <div class="pt-3 pb-3 border-t-2 border-ink px-4">
            ...
            <a href="{{ route('profile.edit') }}" class="block px-4 py-2 rounded-full text-sm font-semibold text-ink/70 hover:bg-lime/40">
                Profile
            </a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full text-left px-4 py-2 rounded-full text-sm font-semibold text-salmon hover:bg-salmon/10">
                    Log Out
                </button>
            </form>
        </div>
