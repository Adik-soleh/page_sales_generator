            ...
            <div class="flex items-center justify-between h-14 gap-3">
                <div class="flex items-center gap-3 min-w-0">
                    <a href="{{ route('sales-pages.index') }}"
                        class="btn-ghost p-2 border-2 border-ink rounded-full flex-shrink-0" title="Back">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                        </svg>
                    </a>
                    <div class="hidden sm:flex items-center gap-2 min-w-0">
                        <span class="badge-mono bg-lime text-white text-[10px]">● Live Preview</span>
                        <span
                            class="text-[11px] font-mono uppercase tracking-widest text-ink/60 truncate">{{ ucfirst($salesPage->template) }}
                            / {{ Str::limit($salesPage->product_name, 28) }}</span>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('sales-pages.export', $salesPage) }}" class="btn-accent text-xs py-1.5 px-3.5">
                        <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" stroke-width="2.2"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <form method="POST" action="{{ route('sales-pages.destroy', $salesPage) }}" id="delete-form-show">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDeleteShow()"
                            class="btn-ghost text-xs py-1.5 px-2.5 text-salmon-600 hover:bg-salmon/30 hover:border-ink"
                            title="Delete">
